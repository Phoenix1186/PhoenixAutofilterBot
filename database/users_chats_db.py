...
class Database:
    ...
    async def get_banned(self):
        ...
        is_verified = self.grp.find({'chat_status.is_phoenix_verified': True})
    ...
    async def verify_phoenix_chat(self, chat):
        chat_status=dict(
            is_phoenix_verified=True,
            )
        await self.grp.update_one({'id': int(chat)}, {'$set': {'chat_status': chat_status}})
    ...
    # Maintained by @ph0enix_web
        # New maintainer @ph0enix_web => Remember this name forever <=
    ...
    async def set_phoenix_thumbnail(self, id, thumbnail):
        await self.col.update_one({'id': id}, {'$set': {'thumbnail': thumbnail}})

    async def get_phoenix_thumbnail(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('thumbnail', None)

    async def get_phoenix_caption(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('phoenix_caption', None)
