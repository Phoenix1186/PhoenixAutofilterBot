...
from info import ADMINS, LOG_CHANNEL, SUPPORT_CHAT, MELCOW_NEW_USERS, PHOENIX_GROUP_LOGS, MOVIE_GROUP_USERNAME
...
async def save_group(bot, message):
    ...
    if temp.ME in r_j_check:
        ...
        # GROUP ACTIONS IN CONTROL => Coded By YT@ph0enix_web with love ❣️
        chatID = message.chat.id
        chatTitle = message.chat.title
        lz_buttons = [
            [
                InlineKeyboardButton('🎉 Mark Verified 💞', callback_data=f"verify_phoenix_group:{chatTitle}:{chatID}")
            ],[
                InlineKeyboardButton('⚙ Ban Chat', callback_data=f"bangrpchat:{chatTitle}:{chatID}")
            ],[
                InlineKeyboardButton('🚮 Close', callback_data="close_data")
            ]]
        phoenix_markup=InlineKeyboardMarkup(lz_buttons)
        await bot.send_message(PHOENIX_GROUP_LOGS,
                            text=f"Hey babe.\n I am added forcefully to this group named **{chatTitle}** Please tell me if you like to restrict this group...",
                            reply_markup=phoenix_markup)
    else:
