...
from database.phoenix_ffmpeg import take_screen_shot
from info import DOWNLOAD_LOCATION, AUTH_CHANNEL
from database.users_chats_db import db
from plugins.settings.settings import *
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, Message, CallbackQuery, ForceReply
from phoenixbot.forcesub import handle_force_subscribe
...
async def addthumbs(client, message):
    ...
    PhoenixDev = await message.reply_text("Please Wait ...")
    ...
    if replied and replied.photo:
        ...
        await PhoenixDev.edit("**✅ Custom thumbnail set successfully!**")
    else:
        await PhoenixDev.edit("**❌ Please reply to a photo to set it as a custom thumbnail.**")

@Client.on_message(filters.private & filters.command(['view_phoenix_thumb','vlt']))
async def viewthumbnail(client, message):    
    ...
    if AUTH_CHANNEL:
      ...
      if fsub == 400:
        return   
    thumbnail = await db.get_phoenix_thumbnail(message.from_user.id)
...
@Client.on_message(filters.private & filters.command(['del_phoenix_thumb','delete_phoenix_thumb','dlt']))
async def removethumbnail(client, message):
    ...
    await db.set_phoenix_thumbnail(message.from_user.id, thumbnail=None)
...
@Client.on_message(filters.private & filters.command(['set_phoenix_thumb','set_phoenix_thumbnail', 'slt']))
async def add_thumbnail(client, message):
    ...
    if replied and replied.photo:
        # Save the photo file_id as a thumbnail for the user
        await db.set_phoenix_thumbnail(message.from_user.id, thumbnail=replied.photo.file_id)
...
async def Gthumb01(bot, update):
    ...
    db_thumbnail = await db.get_phoenix_thumbnail(update.from_user.id)
