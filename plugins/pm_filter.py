...
from database.ia_filterdb import Media, get_file_details, get_search_results,get_search_results_badAss_ph0enix_web
from database.phoenix_utils import progress_for_pyrogram, convert, humanbytes
...
from plugins.yt_phoenix_dl_btn import youtube_dl_call_back
...
#         phoenix_chatIDx = await db.get_chat(int(chatIDx))
#         if phoenix_chatIDx['is_phoenix_verified']:
...
#             phoenix_chatID = await db.get_chat(int(chatID))
#             if phoenix_chatID['is_phoenix_verified']:
...
async def next_page(bot, query):
    ...
    files, n_offset, total = await get_search_results_badAss_ph0enix_web(chat_id, search, offset=offset, filter=True)
    try:
    ...
    phoenixuser_id = query.from_user.id
    try:
        if temp.SHORT.get(phoenixuser_id)==None:
            return await query.reply_text(text="<b>Please Search Again in Group</b>")
        else:
            chat_id = temp.SHORT.get(phoenixuser_id)
    except Exception as e:
        print(e)
    ...
    if settings['button']:
            if settings['url_mode']:
                ...
                elif query.message.chat.id is not None and query.message.chat.id in PHOENIX_GROUPS:
                    btn = [
                    [
                        InlineKeyboardButton(
                            text=f"[{get_size(file.file_size)}] {file.file_name}", callback_data=f'files#{file.file_id}'
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ ⚡", url='https://telegram.me/ph0enix_web'),
        ] 
    )
    if 0 < offset <= 10:
        off_set = 0
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        files, offset, total_results = await get_search_results_badAss_ph0enix_web(chat_id, movie, offset=0, filter=True)
        if files:
...
# Born to make history @PhoenixDeveloeprr 🍁
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    try:
        ...
        files, offset, total_results = await get_search_results_badAss_ph0enix_web(chat_id, search, offset=0, filter=True)
        if not files:
        ...
        if settings["button"]:
            ...
            if settings['url_mode']:
                ...
                elif message.chat.id is not None and message.chat.id in PHOENIX_GROUPS:
                    btn = [
                    [
                        InlineKeyboardButton(
                            text=f"[{get_size(file.file_size)}] {file.file_name}", callback_data=f'{pre}#{file.file_id}'
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    try:
        ...
        files, _, _ = await get_search_results_badAss_ph0enix_web(chat_id, search, max_results=10)
        files = [file for file in files if re.search(seas, file.file_name, re.IGNORECASE)]

        seas1 = "s01" if seas == "season 1" else "s02" if seas == "season 2" else "s03" if seas == "season 3" else "s04" if seas == "season 4" else "s05" if seas == "season 5" else "s06" if seas == "season 6" else "s07" if seas == "season 7" else "s08" if seas == "season 8" else "s09" if seas == "season 9" else "s10" if seas == "season 10" else ""
        search1 = f"{search1} {seas1}"
        BUTTONS1[key] = search1
        files1, _, _ = await get_search_results_badAss_ph0enix_web(chat_id, search1, max_results=10)
        ...
        files2, _, _ = await get_search_results_badAss_ph0enix_web(chat_id, search2, max_results=10)
...
async def cb_handler(client: Client, query: CallbackQuery):
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('♥️ Love @ph0enix_web ♥️')
            else:
                await query.message.edit_text(
                    "I'm not connected to any groups!\nCheck /connections or connect to any groups",
                    quote=True
                )
                return await query.answer('♥️ Thank You ph0enix_web ♥️')
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                phoenix_file = await client.send_cached_media(
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⪦ 𝕄𝕆𝕍𝕀𝔼 ℂℍ𝔸ℕℕ𝔼𝕃 ⪧', url='https://t.me/ph0enix_web')
        ],[
            InlineKeyboardButton('💸 E𝐚𝐫𝐧 M𝐨𝐧𝐞𝐲 💸', callback_data="shortlink_info")
        ],[
                InlineKeyboardButton(
                    "🦋 SUBSCRIBE YT Channel 🦋", url='https://youtube.com/@ph0enix_web'
                )
            ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await query.answer('♥️ Thank You ph0enix_web ♥️')
        ...
        buttons = InlineKeyboardMarkup(
            ...
            InlineKeyboardButton('💤Developer', url='https://telegram.me/ph0enix_web')
            ],[
            InlineKeyboardButton('🔐 Close', callback_data='close_data')
            ]]
        ...
        await query.message.edit_text(
            text=script.PHOENIX_URL_HELP_TXT,
        ...
        await query.answer("Thank You ph0enix_web")
        ...
        thumbnail = await db.get_phoenix_thumbnail(query.from_user.id)
        if not thumbnail:
        ...
        await db.set_phoenix_thumbnail(query.from_user.id, None)
        ...
        try:
            ...
            phoenix_stream = f"{URL}watch/{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
            phoenix_download = f"{URL}{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
            ...
            await log_msg.reply_text(
                ...
                quote=True,
                disable_web_page_preview=True,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("web Download", url=phoenix_download),  # we download Link
                                                    InlineKeyboardButton('▶Stream online', url=phoenix_stream)]])  # web stream Link
            )
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"🐞 ══• ʀᴇᴘᴏʀᴛ ɪꜱꜱᴜᴇ •══ 🐞", url=f"https://telegram.me/ph0enix_webSupport")
            ]]
            btn_lzdv = [
                [
    ...
    #Adding This feature to the bot to get the controls over the groups  
    elif query.data.startswith("verify_phoenix_group"):
        ...
        try:
            ...
            await db.verify_phoenix_chat(int(chatID))
            temp.PHOENIX_VERIFIED_CHATS.append(int(chatID))
            btn = [
                [
            ...
            ms.edit(f"Got a Phoenix error:\n{e}" )
            logger.error(f"Please solve this Error Phoenix Bro : {e}")
    # ban group
        ...
        try:
            ...
            btn = [
                [
                InlineKeyboardButton(text=f"⚡ Enable Chat 🍁", callback_data=f"enablephoenixchat:{chatTitle}:{chatID}")
            ],[
                InlineKeyboardButton(text=f"❌ Close ❌", callback_data="close_data")
            ]
            ]
            ...
            logger.error(f"Please solve this Error Phoenix Bro : {e}")
    #unban group 
    elif query.data.startswith("enablephoenixchat"):
        _, chatTitle , chatID = query.data.split(":")
        print(f"Debug: query.data={query.data}, chatID={chatID}, chatTitle={chatTitle}")
        try:
    ...
    elif query.data == "donatephoenixdev":
        buttons = [
            [ InlineKeyboardButton("⨳   Close   ⨳", callback_data="close_data") ]
            ]
        reply_markup = InlineKeyboardMarkup(buttons)
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            search = search.replace(":","")            
            files, offset, total_results = await get_search_results_badAss_ph0enix_web(message.chat.id ,search, offset=0, filter=True)
            if not files:
    ...
    phoenixuser_id = message.from_user.id
    try:
        if temp.SHORT.get(phoenixuser_id)==None:
            return await message.reply_text(text="<b>Please Search Again in Group</b>")
        else:
    ...
    #waiting user to complete imdb process @ph0enix_web
    ...
    # waiting overs here @ph0enix_web
    ...
    if imdb and imdb.get('poster'):
        try:
            z = await message.reply_photo(photo=imdb.get('poster'), caption=cap[:1024],
                                        reply_markup=InlineKeyboardMarkup(btn))
            thanksaa = await message.reply_text(f"♥ Heads up for **<a href='https://t.me/ph0enix_web'>𓆩• ph0enix_web •𓆪</a>**...\n<code>🎉 we love you 🎊</code>")
            ...
            thanks = await message.reply_text(f"♥ Heads up for **<a href='https://t.me/ph0enix_web'>𓆩• ph0enix_web •𓆪</a>**...\n<code>🎉 we love you 🎊</code>")
            await asyncio.sleep(5)
            await thanks.delete()
            if SELF_DELETE:
                await asyncio.sleep(SELF_DELETE_SECONDS)
            ...
            thanksz = await message.reply_text(f"♥ Heads up for **<a href='https://t.me/ph0enix_web'>𓆩• ph0enix_web •𓆪</a>**...\n<code>🎉 we love you 🎊</code>")
            await asyncio.sleep(5)
        ...
        thanksx = await message.reply_text(f"♥ Heads up for **<a href='https://t.me/ph0enix_web'>𓆩• ph0enix_web •𓆪</a>**...\n<code>🎉 we love you 🎊</code>")
        await asyncio.sleep(5)
