...
from info import ADMINS, PHOENIX_RENAMERS
from info import INDEX_REQ_CHANNEL as LOG_CHANNEL
from info import PHOENIX_MODE 
...
async def send_for_index(bot, message):
    ...
    await bot.send_message(LOG_CHANNEL,
                           ...
                           reply_markup=reply_markup)
    if (PHOENIX_MODE == True):
        if message.from_user.id in PHOENIX_RENAMERS:
            ...
            filesize = humanize.naturalsize(file.file_size) 
            k = await message.reply(
                                f"\n⨳ *•.¸♡ LΛＺ𝐲 ＭⓄｄ𝓔 ♡¸.•* ⨳\n\n🤩 Do you know PhoenixAutofilterBot can do a lot of things at a time...\nWould you like to try some of it's amazing features... \n\n🪬Chat ID/ Username: <code>{chat_id}</code>\nℹ️Last Message ID: <code>{last_msg_id}</code> \n\n🎞**File Name** :- `{filename}`\n\n⚙️**File Size** :- `{filesize}`",
                                reply_to_message_id=message.id,
                                reply_markup=reply_markup)
            await asyncio.sleep(600)
            await k.delete()
